...
class Fake_news_classifier_controller():
    def __init__(self, model_path, in_features, preprocessor_controller, device='cpu'):
        ...
        self.model.eval()
        self.preprocessor_controller = preprocessor_controller
    ...
    def display_classification_report(self):
        # Sample dataset (you can replace this with an actual test dataset)
        sample_texts = ["Some real news", "Some fake news"]
        sample_titles = ["Real news title", "Fake news title"]
        labels = [1, 0]  # 1 for real, 0 for fake

        tensor = self.preprocessor_controller.transform(text_articles=sample_texts, article_titles=sample_titles)

        predictions = self.__predict_tensor(tensor)
        pred_labels = torch.argmax(predictions, dim=1).tolist()

        # Print the classification report
        report = classification_report(labels, pred_labels, target_names=['Fake', 'Real'])
